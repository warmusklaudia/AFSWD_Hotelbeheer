...
export class RequestedServicesResolver {
  ...
  async createRequestedService(
    ...
    if (rs) this.notificationsGateway.server.emit('hotel:requestedServices', rs)
  ...
  @Query(() => [RequestedService], { name: 'unresolvedRequestedServices' })
  findUnresolved(): Promise<RequestedService[]> {
    return this.requestedServicesService.findUnresolved()
  }
