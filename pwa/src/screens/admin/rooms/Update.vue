<template>
  <div class="bg-themeWhite flex h-full">
    <section class="flex h-full w-full">
      <admin-navigation />
      <div class="w-5/6 p-6">
        <admin-header :name="`Edit room ${result?.room.id}`" />
...
export default {
  ...
  setup() {
    ...
    watch(result, () => {
      roomInput.name = result.value?.room.name!
      roomInput.description = result.value?.room.description!
      roomInput.rating = result.value?.room.rating!
      roomInput.category = result.value?.room.category!
      roomInput.location = result.value?.room.location!
      roomInput.accessCode = result.value?.room.accessCode!
    })

    const { mutate: updateRoom } = useMutation(UPDATE_ROOM, () => ({
      variables: {
        updateRoomInput: roomInput,
      },
    }))
    ...
    const submitForm = async () => {
      ...
      await updateRoom()
        ...
        .finally(() => {
          load.value = false
          successMessage.value = 'Room successfully updated'
        })
    }
